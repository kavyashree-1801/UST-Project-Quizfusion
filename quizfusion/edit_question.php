...
// Get category from quizzes table
...
if($_SERVER['REQUEST_METHOD']=='POST') {
    ...
    $stmt = $con->prepare("
        UPDATE questions SET 
            ...
            answer=?,
            hint=?,
            " . ($category=='Audio' ? "audio" : ($category=='Pictionary' ? "image" : "audio")) . " = ?
        WHERE id=?
    ");
...
footer{background:#1a73e8; color:white; text-align:center; padding:12px; flex-shrink:0;}
</style>
</head>
<body>
...
<form method="POST">
    ...
    <select name="answer" required>
        ...
        <option value="<?= htmlspecialchars($question['option1']) ?>" <?= $question['answer']==$question['option1']?'selected':'' ?>><?= htmlspecialchars($question['option1']) ?></option>
        <option value="<?= htmlspecialchars($question['option2']) ?>" <?= $question['answer']==$question['option2']?'selected':'' ?>><?= htmlspecialchars($question['option2']) ?></option>
        <option value="<?= htmlspecialchars($question['option3']) ?>" <?= $question['answer']==$question['option3']?'selected':'' ?>><?= htmlspecialchars($question['option3']) ?></option>
        <option value="<?= htmlspecialchars($question['option4']) ?>" <?= $question['answer']==$question['option4']?'selected':'' ?>><?= htmlspecialchars($question['option4']) ?></option>
    </select>
    ...
    <?php elseif($category=='Pictionary'): ?>
