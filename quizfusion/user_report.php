...
")) {
    $q->bind_param("s", $user_email);
    $q->execute();
    $q->bind_result($catName, $catCount);
    if ($q->fetch()) {
        $topCategory = $catName;
    }
    $q->close();
...
.container{
    ...
    flex:1; /* makes container expand to fill space */
}
...
<nav>
    ...
    <div class="email-text"><?php echo htmlspecialchars($user_email); ?></div>
</nav>
        ...
        <div class="card">
            ...
            <p><?php
                if (!empty($recentAttempts)) {
                    echo htmlspecialchars($recentAttempts[0]['score']).'%';
                } else {
                    echo "—";
                }
            ?></p>
