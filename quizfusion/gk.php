...
if (!isset($_SESSION['gk_questions'])) {
    shuffle($all_questions);
    $_SESSION['gk_questions'] = $all_questions;
}

$questions = $_SESSION['gk_questions'];

// --------------------------------------------
// 4. QUESTION INDEX
// --------------------------------------------
if (!isset($_SESSION['q_index_gk'])) {
    $_SESSION['q_index_gk'] = 0;
    $_SESSION['score_gk'] = 0;
}

$index = $_SESSION['q_index_gk'];
...
if (isset($_POST['next'])) {
    ...
    if ($index >= $total) {
        ...
        ");
        $quiz_finished = true;
        unset($_SESSION['q_index_gk']);
        unset($_SESSION['score_gk']);
        unset($_SESSION['gk_questions']);
    } else {
        $current = $questions[$index];
        $selected_option = null;
        $show_next = false;
...
body {
    ...
    background: url('https://e1.pxfuel.com/desktop-wallpaper/449/790/desktop-wallpaper-general-knowledge-knowledge.jpg') no-repeat center center fixed;
