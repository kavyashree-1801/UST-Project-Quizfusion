...
// --------------------------------------------
// 2. FETCH QUIZ ID FOR CATEGORY 'Technical'
// --------------------------------------------
$category = "Technical";
...
if (!isset($_SESSION['technical_questions'])) {
    shuffle($all_questions);
    $_SESSION['technical_questions'] = $all_questions;
}

$questions = $_SESSION['technical_questions'];

// --------------------------------------------
// 4. QUESTION INDEX
// --------------------------------------------
if (!isset($_SESSION['q_index_tech'])) {
    $_SESSION['q_index_tech'] = 0;
    $_SESSION['score_tech'] = 0;
}

$index = $_SESSION['q_index_tech'];
...
if (isset($_POST['next'])) {
    ...
    if ($index >= $total) {
        ...
        ");
        $quiz_finished = true;
        unset($_SESSION['q_index_tech']);
        unset($_SESSION['score_tech']);
        unset($_SESSION['technical_questions']);
    } else {
        $current = $questions[$index];
        $selected_option = null;
        $show_next = false;
...
body {
    ...
    background: url('https://thumbs.dreamstime.com/b/futuristic-ai-quiz-app-interface-holographic-visuals-robotic-assistant-icon-powered-featuring-sleek-metallic-background-350603544.jpg') no-repeat center center fixed;
...
foreach(['option1','option2','option3','option4'] as $opt){
    ...
    if($selected_option){
        if($value == $current['answer']) {
            ...
            $class = 'correct';
        } elseif($value == $selected_option && $selected_option != $current['answer']) {
