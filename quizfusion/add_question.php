...
if ($quiz_id == '') die("Quiz ID not specified.");

// Fetch quiz info
$stmt_q = $con->prepare("SELECT category FROM quizzes WHERE id=?");
$stmt_q->bind_param("i", $quiz_id);
$stmt_q->execute();
$stmt_q->bind_result($category);
$stmt_q->fetch();
$stmt_q->close();

if (!$category) die("Invalid Quiz ID.");
...
if (isset($_POST['submit'])) {
    ...
    // Audio: store audio link
    if (strtolower($category) == 'audio' && !empty($_POST['audio_link'])) {
        $audio_file_name = trim($_POST['audio_link']);
    }
...
<script>
// Update answer select dynamically
